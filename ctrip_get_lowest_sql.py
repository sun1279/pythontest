...
connection = sqlite3.connect("FlightTable.db") 
crsr = connection.cursor() 
sql_cmd="""CREATE TABLE flight (
create_data varchar(8),
dep_info varchar(30),
air varchar(20),
flightno varchar(10),
date varchar(8),
...
for l in ls:
    if Sort is 0:
        ...
        dep=DepCity+'-'+ArrCity
        flightno=l.get('flightNo')
        air=l.get('airname')
        date=l.get('dDate')
        week=l.get('dweek')
        price=l.get('price')
        sql_command = "INSERT INTO flight  (create_data,dep_info,air,flightno,date,week,price) VALUES (?,?,?,?,?,?,?)"
        crsr.execute(sql_command,(ctime, dep, flightno, air, date, week, price))
